...
{
	CClientFog* pFog = GetClassPtr<CCSClientFog>((CClientFog*)NULL);
	pFog->pev->classname = MAKE_STRING("env_fog_te");
	pFog->Spawn();
...
void CClientFog::KeyValue(KeyValueData* pkvd)
{
	...
	{
		m_bDontAffectSky = Q_atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
		CBaseEntity::KeyValue(pkvd);
...
LINK_ENTITY_TO_CLASS(item_generic, CItemGeneric, CCSItemGeneric)
LINK_ENTITY_TO_CLASS(item_prop, CItemGeneric, CCSItemGeneric)
...
void CItemGeneric::KeyValue(KeyValueData* pkvd)
{
	...
	{
		m_fDisableShadows = Q_atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "DisableDrawing"))
	{
		m_fDisableDrawing = Q_atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
...
LINK_ENTITY_TO_CLASS(env_elight, CDynamicLight, CCSDynamicLight)
LINK_ENTITY_TO_CLASS(env_dlight, CDynamicLight, CCSDynamicLight)
LINK_ENTITY_TO_CLASS(env_spotlight, CDynamicLight, CCSDynamicLight)
...
LINK_ENTITY_TO_CLASS(envpos_sky, CEnvPos_Sky, CCSEnvPos_Sky)
...
LINK_ENTITY_TO_CLASS(envpos_world, CEnvPos_World, CCSEnvPos_World)
...
TYPEDESCRIPTION	CEnvDecal::m_SaveData[] =
{
	...
	DEFINE_ARRAY(CEnvDecal, m_szDecalName, FIELD_CHARACTER, MAX_DECALPATH_LENGTH),
};
IMPLEMENT_SAVERESTORE(CEnvDecal, CBaseEntity)

LINK_ENTITY_TO_CLASS(env_decal, CEnvDecal, CCSEnvDecal)
...
LINK_ENTITY_TO_CLASS(env_particle_system, CEnvParticle, CCSEnvParticle)
