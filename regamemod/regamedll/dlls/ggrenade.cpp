...
void CGrenade::__API_HOOK(Explode3)(TraceResult *pTrace, int bitsDamageType)
{
	...
	else
	{
		...
		{
			// particle
			UTIL_Particle("water_explosion_cluster.txt", tr.vecEndPos + Vector(0, 0, 5), Vector(0, 0, 2), 1);

			// play sound 
			UTIL_EmitAmbientSound(ENT(0), tr.vecEndPos + Vector(0, 0, 5), "misc/water_explosion.wav", 3, ATTN_NORM, 0, 100);
		}
	}
	//RENDERERS END
