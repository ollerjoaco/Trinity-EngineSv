...
void CBSPRenderer::GetRenderEnts(void)
{
	...
	for (int i = 1; i < MAXRENDERENTS; i++)
	{
		...
		// This needs a fix to add waters draw as world brushes ( not defined as func_water but with a texture starting with ! or water_ )
		if (((pEntity->curstate.effects & FL_WATERSHADER) != 0 && m_bShaderSupport && gWaterShader.m_pCvarWaterShader->value > 0) || ((pEntity->curstate.skin == CONTENTS_WATER) != 0 && m_bShaderSupport && gWaterShader.m_pCvarWaterShader->value > 0))
		{
			{
...
{
	// CHECK : Try to disable Picture In Picture mode too as it´s bugged rn
...
void CBSPRenderer::CreateDecal(vec3_t endpos, vec3_t pnormal, const char *name, int persistent)
{
	...
	else
	{
		for (int i = 0; i < m_iNumDecals; i++)
		{
			...
			if (!CullDecalBBox(mins, maxs))		// AVERIGUAR QUE ES ESTO Y POR QUE VENIA COMENTADO
			{
